...
namespace ICC
{
	namespace PROTOCOL
	{
		class CSearchLogoutCount : public IRequest
		{
		public:
		...
		public:
			virtual std::string ToString(JsonParser::IJsonPtr p_pJson)
			{
				...
				p_pJson->SetNodeValue("/body/total_logout_count", m_oBody.m_strSum);
				...
				for (size_t i = 0; i < m_oBody.m_vecData.size(); i++)
				{
					...
					p_pJson->SetNodeValue("/body/data/" + l_strIndex + "/logout_count", m_oBody.m_vecData[i].m_strLogoutSum);
				}
