        ...
        public:
            bool ParseString(std::string p_strJson, JsonParser::IJsonPtr p_pJson)
            {
                ...
                for (int i = 0; i < p_pJson->GetCount("/body/data"); i++)
                {
					...
					l_CData.m_strPucID= p_pJson->GetNodeValue(l_strPath + l_strNum + "/puc_id", "");
					l_CData.m_strPucSysType = p_pJson->GetNodeValue(l_strPath + l_strNum + "/puc_sys_type", "undef");
        ...
        public:
            ...
            public:
                ...
                public:
					...
					std::string m_strPucSysType;	//系统类型，如ICC等
