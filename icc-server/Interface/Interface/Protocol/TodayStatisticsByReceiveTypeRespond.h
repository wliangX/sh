...
namespace ICC
{
	namespace PROTOCOL
	{
		class CTodayStatisticsByReceiveTypeRespond :
			public IRespond
		{
		public:
			virtual std::string ToString(JsonParser::IJsonPtr p_pJson)
			{
				...
				for (size_t i = 0; i < iCount; ++i)
				{
					...
					p_pJson->SetNodeValue("/body/data/" + l_strIndex + "/receive_type", m_oBody.m_vecDatas[i].m_strReceiveType);
		...
		public:		
			...
			{
			public:
				std::string m_strReceiveType;  //接警类型				
