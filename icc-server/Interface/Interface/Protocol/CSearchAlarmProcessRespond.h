...
namespace ICC
{
	namespace PROTOCOL
	{
		class CSearchAlarmProcessRespond :
			public IRespond
		{
		public:
			virtual std::string ToString(JsonParser::IJsonPtr p_pJson)
			{
				...
				for (auto iter = m_oBody.m_vData.begin(); iter != m_oBody.m_vData.end(); iter++)
				{
					...
					p_pJson->SetNodeValue(l_strPrefixPath + "centre_process_dept_code", iter->m_ProcessData.m_strCentreProcessDeptCode);
					p_pJson->SetNodeValue(l_strPrefixPath + "centre_alarm_dept_code", iter->m_ProcessData.m_strCentreAlarmDeptCode);
					p_pJson->SetNodeValue(l_strPrefixPath + "dispatch_dept_district_name", iter->m_ProcessData.m_strDispatchDeptDistrictName);
					p_pJson->SetNodeValue(l_strPrefixPath + "linked_dispatch_code", iter->m_ProcessData.m_strLinkedDispatchCode);
					p_pJson->SetNodeValue(l_strPrefixPath + "over_time", iter->m_ProcessData.m_strOverTime);
					p_pJson->SetNodeValue(l_strPrefixPath + "finished_timeout_state", iter->m_ProcessData.m_strFinishedTimeoutState);
					p_pJson->SetNodeValue(l_strPrefixPath + "police_type", iter->m_ProcessData.m_strPoliceType);
					p_pJson->SetNodeValue(l_strPrefixPath + "process_dept_short_name", iter->m_ProcessData.m_strProcessDeptShortName);
					p_pJson->SetNodeValue(l_strPrefixPath + "dispatch_dept_short_name", iter->m_ProcessData.m_strDispatchDeptShortName);
					...
					p_pJson->SetNodeValue(l_strPrefixPath + "received_time", iter->m_ProcessData.m_strReceivedTime);
