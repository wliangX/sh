...
namespace ICC
{
	namespace PROTOCOL
	{
		class CSearchReleaseCount : public IRequest
		{
		public:
			virtual bool ParseString(std::string p_strReq, JsonParser::IJsonPtr p_pJson)
			{
				...
				m_oBody.m_oData.m_strDeptCode = p_pJson->GetNodeValue("/body/dept_code", "");				
		...
		public:
			virtual std::string ToString(JsonParser::IJsonPtr p_pJson)
			{
				...
				p_pJson->SetNodeValue("/body/total_release_count", m_oBody.m_strSum);
				...
				for (size_t i = 0; i < m_oBody.m_vecData.size(); i++)
				{
					...
					p_pJson->SetNodeValue("/body/data/" + l_strIndex + "/release_count", m_oBody.m_vecData[i].m_strReleaseCount);
				}
