...
unsigned int CCommonWorkThread::GetThreadId()
{
	boost::thread::id threadId = m_threadExecMsg->get_id();
	unsigned int idd = *(unsigned int*)&threadId;
	return idd;
}

int CCommonWorkThread::AddMessage(CCommonThreadCore *pThreadCore /* = NULL */, __culong_ uMsgId /* = 0 */, __culong_ ulParam /* = 0 */, 
	__culong_ uwParam /* = 0 */, __clong_ lData /* = 0 */, std::string strmsg /* = "" */, std::string strExt /* = "" */,
	ICC::ObserverPattern::INotificationPtr pTask /* = nullptr */, ICC::PROTOCOL::IRequestPtr pRequest /* = nullptr */, 
	ICC::PROTOCOL::IRespondPtr pRespond /* = nullptr */,  ICC::PROTOCOL::AddOrUpdateAlarmWithProcessRequestPtr pAlarmRequest /* = nullptr */)
{
	try
	{		
		if (NULL != pThreadCore)
		{
			if (m_bStopRunning)
			{
				...
				return GetCWorkThreadListCount();
			}
			...
			msg_data.thread_data.strExt = strExt;
			msg_data.thread_data.pNotifyPtr = pTask;		
			msg_data.thread_data.pRequestPtr = pRequest;
			msg_data.thread_data.pRespondPtr = pRespond;
			msg_data.thread_data.pAlarmRequestPtr = pAlarmRequest;

			_AddMsgToVector(msg_data);			
			
			//m_eventExecWait.notify_all();		
			m_eventExecWait.notify_one();
			...
			if (m_pLog)
			{
				ICC_LOG_LOWDEBUG(m_pLog, "thread %u add msg complete:[%s:%s], total add: %d, total exec %d",
					GetThreadId(), strmsg.c_str(), strExt.c_str(), m_iTotalAddCount.load(),m_iTotalExecCount.load());
			}
...
void CCommonWorkThread::run()
{
	if (m_pLog)
	{
		ICC_LOG_LOWDEBUG(m_pLog, "thread %u running!!!", GetThreadId());
	}	
	...
	while(!m_bStopRunning)
	{
		...
		{			
			...
			{				
				...
				if (m_pLog)
				{
					ICC_LOG_LOWDEBUG(m_pLog, "thread %u exec msg 1:[%s:%s]", GetThreadId(), tmp.thread_data.str_msg.c_str(), tmp.thread_data.strExt.c_str());
				}
				...
				if (NULL != tmp.threadcore)
				{
					try
					{
						if (m_pLog)
						{
							ICC_LOG_LOWDEBUG(m_pLog, "thread %u exec msg 2:[%s:%s]", GetThreadId(), tmp.thread_data.str_msg.c_str(), tmp.thread_data.strExt.c_str());
						}
						

						tmp.thread_data.uThreadId = GetThreadId();
						tmp.threadcore->ProcMessage(tmp.thread_data);

						if (m_pLog)
						{
							ICC_LOG_LOWDEBUG(m_pLog, "thread %u exec msg 3:[%s:%s]", GetThreadId(), tmp.thread_data.str_msg.c_str(), tmp.thread_data.strExt.c_str());
						}
				...
				if (m_pLog)
				{
					ICC_LOG_LOWDEBUG(m_pLog, "thread %u exec msg 4:[%s:%s], total add:%d, total exec: %d",
						GetThreadId(), tmp.thread_data.str_msg.c_str(), tmp.thread_data.strExt.c_str(), m_iTotalAddCount.load(),
						m_iTotalExecCount.load());

				}

				std::chrono::system_clock::time_point curEnd;
				curEnd = std::chrono::system_clock::now();
				unsigned long long ullInterval = std::chrono::duration_cast<std::chrono::milliseconds>(curEnd.time_since_epoch()).count() - std::chrono::duration_cast<std::chrono::milliseconds>(curBegin.time_since_epoch()).count();
			...
			else
			{
				...
				{					
					...
					if (m_pLog)
					{
						ICC_LOG_LOWDEBUG(m_pLog, "thread %u exec msg 5", GetThreadId());
					}
	...
	if (m_pLog)
	{
		ICC_LOG_LOWDEBUG(m_pLog, "thread %u exit!!!", GetThreadId());
	}
