...
//	m_strDialRuleList = "";
	...
	m_nInitFailedMaxCount = 5;
	m_strSlaveAEServerIP = "";
	m_strSlaveAEServerPort = "";
	m_strSlaveAESUserName = "";
	m_strSlaveAESPsw = "";
}
CSysConfig::~CSysConfig(void)
{
...
void CSysConfig::_LoadCommonParam()
{
	{
		std::lock_guard<std::mutex> lock(m_mutexCalloutParam);

		m_strEnbaleConfigCalloutCheck = m_ConfigPtr->GetValue("ICC/Plugin/CTI/EnableConfigCalloutCheck", "1");
...
void CSysConfig::LoadParamter()
{
	if (!m_bParamterIsLoad)
	{
		...
		m_strAEServerIP = m_ConfigPtr->GetValue("ICC/Plugin/TSAPIClient/AEServerIP", "127.0.0.1");
		m_strAEServerPort = m_ConfigPtr->GetValue("ICC/Plugin/TSAPIClient/AEServerPort", "450");
		m_strAESUserName = m_ConfigPtr->GetValue("ICC/Plugin/TSAPIClient/AEServerUserName", "cti");
		m_strAESPsw = m_ConfigPtr->GetValue("ICC/Plugin/TSAPIClient/AEServerPassword", "ctiCTI1!");

		m_strSlaveAEServerIP = m_ConfigPtr->GetValue("ICC/Plugin/SlaveTSAPIClient/AEServerIP", "");
		m_strSlaveAEServerPort = m_ConfigPtr->GetValue("ICC/Plugin/SlaveTSAPIClient/AEServerPort", "450");
		m_strSlaveAESUserName = m_ConfigPtr->GetValue("ICC/Plugin/SlaveTSAPIClient/AEServerUserName", "cti");
		m_strSlaveAESPsw = m_ConfigPtr->GetValue("ICC/Plugin/SlaveTSAPIClient/AEServerPassword", "ctiCTI1!");

		std::string l_strInitFailedMaxCount = m_ConfigPtr->GetValue("ICC/Plugin/CTI/avayaInitFailedMaxCount", "5");
		m_nInitFailedMaxCount = atoi(l_strInitFailedMaxCount.c_str());
		...
		m_strIsUseBlackRoute = m_ConfigPtr->GetValue("ICC/Plugin/CTI/IsUseBlackRoute", "0");
		m_strBlackRouteDest = m_ConfigPtr->GetValue("ICC/Plugin/CTI/BlackRouteDest", "");
		...
		for (int i = 0; i < l_nDeviceNodeCount; ++i)
		{
			...
			if (l_vDevice.size() == l_iCorrectSize)
			{
				...
				/*ICC_LOG_DEBUG(m_pLog, "Load Phone, DeviceNum: [%s], DeviceType: [%s], AgentId: [%s], Psw: [%s]",
					ipPhone.m_strDeviceNum.c_str(), ipPhone.m_strDeviceType.c_str(), ipPhone.m_strAgent.c_str(), ipPhone.m_strPsw.c_str());*/
			}
		}
...
void CSysConfig::_LoadDefaultReadyAgents()
{
	// 默认置闲的坐席列表
	std::string l_strAgentList = m_ConfigPtr->GetValue("ICC/Plugin/CTI/ReadyAgents", "");
	std::vector<std::string> l_vTempAgentList;
	...
	for (auto l_strAgent : l_vTempAgentList)
	{
		if (!l_strAgent.empty())
		{
			std::lock_guard<std::mutex> lock(m_mutexDefaultReadyAgent);
			m_DefaultReadyAgentList.push_back(l_strAgent);
		}
	}

	ICC_LOG_DEBUG(m_pLog, "default ready agents load complete! size: [%d]", m_DefaultReadyAgentList.size());
}
...
std::string CSysConfig::GetLastConnectAvayaIP()
{
	std::string l_lstrAvayaAESServerIP;
	if (m_pRedisClient->HGet("AVAYA_Server", "AES_IP", l_lstrAvayaAESServerIP))
	{
		ICC_LOG_DEBUG(m_pLog, "GetLastConnectAvayaIP,AES_IP:%s", l_lstrAvayaAESServerIP.c_str());
	}
	return l_lstrAvayaAESServerIP;
}

bool CSysConfig::SetCurrConnectAvayaIP(const string& p_lstrAvayaAESServerIP)
{
	std::string l_lstrAvayaAESServerIP = p_lstrAvayaAESServerIP;
	if (!l_lstrAvayaAESServerIP.empty())
	{
		if (m_pRedisClient->HSet("AVAYA_Server", "AES_IP", l_lstrAvayaAESServerIP))
		{
			ICC_LOG_DEBUG(m_pLog, "SetCurrConnectAvayaIP,AES_IP:%s", l_lstrAvayaAESServerIP.c_str());
			return true;
		}
	}
...
{
	std::map<std::string, std::string> mapMergeHuntGrpConvers;
	{
		...
		for (int i = 0; i < l_nCount; i++)
		{
			...
			if (!l_strConverACD.empty() && !l_strNumber.empty())
			{
				...
				mapMergeHuntGrpConvers[l_strNumber] = l_strConverACD;
			}
		}
	}

	{
		std::lock_guard<std::mutex> lock(m_mutexMergeHuntGrpConvers);
		m_mapMergeHuntGrpConvers = mapMergeHuntGrpConvers;
	}
...
bool CSysConfig::IsResonanceGroupNumber(const std::string& strDeviceNumber)
{
	std::lock_guard<std::mutex> lock(m_mutexLoadResonanceGroups);
	return m_mapResonanceGroups.find(strDeviceNumber) != m_mapResonanceGroups.end();
}
...
void CSysConfig::_LoadResonanceGroups()
{
	std::map<std::string,std::string>	mapResonanceGroups;

	int iResonanceGroupCount = m_ConfigPtr->GetNodeCount("ICC/Plugin/CTI/ResonanceGroups", "ResonanceGroup");
	for (int i = 0; i < iResonanceGroupCount; ++i)
	{
		std::string l_stPrePath = "ICC/Plugin/CTI/ResonanceGroups/ResonanceGroup[" + std::to_string(i) + "]";

		std::string strResonanceGroupNumber = m_ConfigPtr->GetValue(l_stPrePath + "/Number", "");			

		ICC_LOG_DEBUG(m_pLog, "Load ResonanceGroups, number: [%s]",	strResonanceGroupNumber.c_str());
		if (!strResonanceGroupNumber.empty())
		{
			mapResonanceGroups.insert(std::make_pair(strResonanceGroupNumber, strResonanceGroupNumber));
		}
	}

	ICC_LOG_DEBUG(m_pLog, "Load ResonanceGroups complete, size: [%d]", mapResonanceGroups.size());

	{
		std::lock_guard<std::mutex> lock(m_mutexLoadResonanceGroups);
		m_mapResonanceGroups = mapResonanceGroups;
	}
}
