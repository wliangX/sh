...
namespace
{
	...
	void writeAnyInt(std::ostream& str, const Any& any)
	{
		if (any.type() == typeid(char))
			...
			str << AnyCast<long long>(any);
		else if (any.type() == typeid(unsigned long long))
			str << AnyCast<unsigned long long>(any);
		...
		try
		{
			...
			if (itFmt != endFmt)
			{
				...
				case 'i':
					...
					case 'L': str << AnyCast<long long>(*itVal++); break;
				...
				case 'X':
					...
					case 'L': str << AnyCast<unsigned long long>(*itVal++); break;
		...
		case '%':
			...
			if (itFmt != endFmt && (itVal != endVal || *itFmt == '['))
			{
				if (*itFmt == '[')
				{
					...
					else throw ("format argument index out of range " + fmt);
				}
				else
				{
