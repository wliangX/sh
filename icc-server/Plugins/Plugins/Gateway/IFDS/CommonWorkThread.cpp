...
void CCommonWorkThread::AddMessage(CCommonThreadCore *pThreadCore /* = NULL */, __culong_ uMsgId /* = 0 */, __culong_ ulParam /* = 0 */, __culong_ uwParam /* = 0 */, __clong_ lData /* = 0 */, std::shared_ptr<void> ptr /* = nullptr */, std::string strmsg /* = "" */)
{
	try
	{		
		if (NULL != pThreadCore)
...
void CCommonWorkThread::Stop(long lMilliseconds)
{
	...
	{
		std::lock_guard<std::mutex> lock(m_mutexAddMsgVector);
