...
/*                                                                      */
/************************************************************************/
std::string CSeatStatusChanngedProcesser::ProcNotify(const std::string& strMsg)
{
	/*std::string strTmpLog;
	PROTOCOL::CSetInOutInfo inoutInfo;
	if (!_ParseMsg(strMsg, inoutInfo))
	{
	return "";
	}

	_UpdateSeatStatus(inoutInfo);
...
/*                                                                      */
/************************************************************************/
CSeatStatusChanngedProcesser::CSeatStatusChanngedProcesser(JsonParser::IJsonFactoryPtr pJsonFty, StringUtil::IStringUtilPtr pStrUtil, DateTime::IDateTimePtr pDateTime, 
...
CSeatStatusChanngedProcesser::~CSeatStatusChanngedProcesser()
{

}

bool CSeatStatusChanngedProcesser::_ParseMsg(const std::string& strMsg, PROTOCOL::CSetInOutInfo& seatInfo)
{
	std::string strTmpLog;
...
void CSeatStatusChanngedProcesser::_UpdateSeatStatus(PROTOCOL::CSetInOutInfo& seatInfo)
{
	if (m_pSeatStorager)
	{
		SeatInfo info;
		info.strNumber = seatInfo.m_oBody.m_strClient;
		info.strAcd = seatInfo.m_oBody.m_strACD;
		info.strLoginMode = seatInfo.m_oBody.m_strLogMode;
		info.strStatus = seatInfo.m_oBody.m_strReadyState;
		info.strUpdateTime = seatInfo.m_oBody.m_strTime;
		m_pSeatStorager->UpdateSeatStatus(info);
	}
}

void CSeatStatusChanngedProcesser::_PacketHeader(PROTOCOL::CHeader& header)
{
...
void CSeatStatusChanngedProcesser::_PacketBody(const std::vector<SeatInfo>& vecSeats, PROTOCOL::CIfdsSeatInfo::CBody& body)
{
