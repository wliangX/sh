...
CPGClientExImpl::CPGClientExImpl(IResourceManagerPtr p_pResourceManager)
...
CPGClientExImpl::~CPGClientExImpl()
{

}

bool CPGClientExImpl::ConnectServer()
{
	return PostgresqlPool::Instance()->Init(m_pResourceManager);
}

void CPGClientExImpl::Close()
{
	PostgresqlPool::Instance()->CloseConnectionPool();
}

unsigned int CPGClientExImpl::ExecQuery(std::string p_strSql, std::map<std::string, std::string>& p_mapFieldInfo, std::list<std::list<std::string>>& p_lstRes, bool bIsRead /* = false */, std::string strTransGuid /* = "" */)
{
...
IResultSetPtr CPGClientExImpl::ExecQuery(std::string p_strSql, bool bIsRead /* = false */, std::string strTransGuid /* = "" */)
{
...
unsigned int CPGClientExImpl::ExecNoQuery(std::string p_strSql, bool bIsRead /* = false */, std::string strTransGuid /* = "" */)
{
	PGresult* pRet = nullptr;
	PooledConnectionPtr pPooledConnection = _GetConnection(bIsRead, strTransGuid);
	if (!pPooledConnection)
...
std::string CPGClientExImpl::BeginTransaction()
{
	PGresult* pRet = nullptr;
...
unsigned int CPGClientExImpl::Commit(std::string p_strGuid)
{
...
std::string CPGClientExImpl::Savepoint(std::string p_strSavePoint)
{
	PGresult* pRet = nullptr;
	PooledConnectionPtr pPooledConnection = PostgresqlPool::Instance()->GetFreeRWConnection();
	if (!pPooledConnection)
...
unsigned int CPGClientExImpl::Rollback(std::string p_strGuid, std::string p_strSavePoint /* = "" */)
{
	PooledConnectionPtr pConn;
	{
		m_Mutex.lock();
...
std::string CPGClientExImpl::GetFieldType(Oid p_uiType)
{
...
IResourceManagerPtr CPGClientExImpl::GetResourceManager()
{
	return m_pResourceManager;
}

PooledConnectionPtr CPGClientExImpl::_GetConnection(bool bIsRead, const std::string& strTransGuid)
{
