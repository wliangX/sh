...
CPGConnExImpl::CPGConnExImpl(IResourceManagerPtr p_pResourceManager):
	m_pResourceManager(p_pResourceManager)
{
	m_pDBClient = ICCGetIPGClientExFactory()->CreatePGClient();
...
CPGConnExImpl::~CPGConnExImpl()
{

}

bool CPGConnExImpl::Connect()
{
	return m_pDBClient->ConnectServer();
}

void CPGConnExImpl::Close()
{
	m_pDBClient->Close();
}
...
//************************************
IResultSetPtr CPGConnExImpl::Exec(std::string p_strSql, bool bIsRead /* = false */, std::string strTransGuid /* = "" */)
{
 ...
 * 作者：[3/2/2018 t26150]
*/
IResultSetPtr CPGConnExImpl::Exec(DataBase::SQLRequest& p_oSqlReq, bool bIsRead /* = false */, std::string strTransGuid /* = "" */)
{
...
std::string CPGConnExImpl::BeginTransaction()
{
	return m_pDBClient->BeginTransaction();
}

unsigned int CPGConnExImpl::Commit(std::string p_strGuid)
{
	return m_pDBClient->Commit(p_strGuid);
}

std::string CPGConnExImpl::Savepoint(std::string p_strSavePoint)
{
	return m_pDBClient->Savepoint(p_strSavePoint);
}

unsigned int CPGConnExImpl::Rollback(std::string p_strGuid, std::string p_strSavePoint /* = "" */)
{
	return m_pDBClient->Rollback(p_strGuid, p_strSavePoint);
}

ICC::IResourceManagerPtr CPGConnExImpl::GetResourceManager()
{
