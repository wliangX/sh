...
bool PostgresqlPool::Init(IResourceManagerPtr p_pResourceManager)
{
	...
	m_lockSaved = ICCGetILockFactory()->CreateLock(Lock::TypeMutex);
	...
	m_strDBName = l_pConfig->GetValue("ICC/Component/DBConn/dbname", "icc");
	m_iInitNum = m_pString->ToUInt(l_pConfig->GetValue("ICC/Component/DBConn/connnum", "10"));
	m_iIncreatmentNum = m_pString->ToUInt(l_pConfig->GetValue("ICC/Component/DBConn/increasenum", "2"));
	m_iMaxNum = m_pString->ToUInt(l_pConfig->GetValue("ICC/Component/DBConn/maxnum", "20"));
	m_strStatementTimeout = l_pConfig->GetValue("ICC/Component/DBConn/statementtimeout", "30000");

	if (m_iInitNum < 0 || m_iInitNum > 1000)
	{
		m_iInitNum = 10;
	}

	if (m_iMaxNum < 0 || m_iMaxNum > 1000)
	{
		m_iMaxNum = 20;
	}

	m_iCommonNum = (m_iInitNum + m_iMaxNum) / 2;

	ICC_LOG_DEBUG(m_pLog, "pg statement timeout = %s, init num:%d, max num:%d,common num:%d", m_strStatementTimeout.c_str(), m_iInitNum, m_iMaxNum, m_iCommonNum);

	//先从公共配置中读取
	int nPubHostCount = l_pConfig->GetPubicNodeCount("ICC/Component/DBConn/Servers", "Server");
	...
	{
		int iHostCount = l_pConfig->GetNodeCount("ICC/Component/DBConn/Servers", "Server");
		for (int i = 0; i < iHostCount; ++i)
		{
			std::string strPath = m_pString->Format("ICC/Component/DBConn/Servers/Server[%d]", i);
	...
	ICC_LOG_DEBUG(m_pLog, "postgresql pool init complete. createtime = %llu", dtcur - dt);
...
void PostgresqlPool::CheckDBPool(PostgresqlPool* p_pThread)
{
	...
	while (1)
	{
		...
		if (strCurrentMasterHostIp != strSavedMasterHostIp && !strCurrentMasterHostIp.empty())
		{
		...
		{
			Lock::AutoLock l_lock(m_lockSaved);

			if (m_vecSavedRConnections.size() > 0)
			{
				bool bHaveBusy = false;
				for (auto var : m_vecSavedRConnections)
				{
					if (!var->GetIsBusy())
					{
...
PooledConnectionPtr	PostgresqlPool::GetFreeRWConnection()
{
	...
	ptrReturnConn = _FindFreeRWConnection();
	if (ptrReturnConn == nullptr)//目没有可用的连接
	{
	...
	ICC_LOG_DEBUG(m_pLog, "current rw connection size[%d]", m_RWConnections.size());

	PooledConnectionPtr pTmpConn = nullptr;
	if (ptrReturnConn != nullptr)
	{
		pTmpConn = boost::make_shared<PooledConnection>(ptrReturnConn->Get());
	}
...
PooledConnectionPtr	PostgresqlPool::GetFreeRConnection()
{
	Lock::AutoLock l_lock(m_lockR);
	PooledConnectionPtr ptrReturnConn = nullptr;
	ptrReturnConn = _FindFreeRConnection();
	if (ptrReturnConn == nullptr)//目没有可用的连接
	{
	...
	ICC_LOG_DEBUG(m_pLog, "current r connection size[%d]", m_RConnections.size());
...
PGconn* PostgresqlPool::_NewConnection(const std::string& strConnectionInfo)
{
	...
	std::string strTmpSql = m_pString->Format("SET statement_timeout = %s;", m_strStatementTimeout.c_str());
	PGresult* l_pPgRes = PQexec(pConn, strTmpSql.c_str());
	if (l_pPgRes)
	{
		unsigned int l_uiExecStatusType = PQresultStatus(l_pPgRes);
		if (l_uiExecStatusType != PGRES_FATAL_ERROR && l_uiExecStatusType != PGRES_BAD_RESPONSE)
		{
			ICC_LOG_DEBUG(m_pLog, "set statement_timeout success!");
		}
		else
		{
			ICC_LOG_ERROR(m_pLog, "set statement_timeout failed!");
		}
	}
	else
	{
		ICC_LOG_ERROR(m_pLog, "set statement_timeout result is null!");
	}
...
void PostgresqlPool::_ReturnRWConnection(PGconn* pConn)
{
	...
	int iFreeSize = 0;
	{
		...
		ICC_LOG_DEBUG(m_pLog, "_ReturnRWConnection current rw freesize:[%d]", iFreeSize);

		if (iFreeSize > m_iCommonNum)
		{
			std::vector<PooledConnectionPtr>::iterator itr;
			for (itr = m_RWConnections.begin(); itr != m_RWConnections.end(); )
			{
				if (pConn == (*itr)->Get())
				{
					_CloseConnection(pConn);
					(*itr)->Set(nullptr);
					(*itr)->SetNeedFree(false);
					m_RWConnections.erase(itr++);
					break;
				}
				else
				{
	...
	ICC_LOG_LOWDEBUG(m_pLog, "_ReturnRWConnection complete current rw freesize:[%d], exec time:%llu, ", iFreeSize, ullInterval);
...
void PostgresqlPool::_ReturnRConnection(PGconn* pConn)
{
	...
	bool bFind = false;
	{
		...
		ICC_LOG_DEBUG(m_pLog, "_ReturnRConnection current r freesize:[%d]", iFreeSize);

		if (iFreeSize > m_iCommonNum)
		{
			std::vector<PooledConnectionPtr>::iterator itr;
			for (itr = m_RConnections.begin(); itr != m_RConnections.end(); )
			{
				if (pConn == (*itr)->Get())
				{					
					_CloseConnection(pConn);
	...
	if (!bFind)
	{
		...
		if (bFind)
		{
			ICC_LOG_WARNING(m_pLog, "find connection in saved r connections, ");
		}
	}
	...
	curEnd = std::chrono::system_clock::now();
	unsigned long long ullInterval = std::chrono::duration_cast<std::chrono::microseconds>(curEnd.time_since_epoch()).count() - std::chrono::duration_cast<std::chrono::microseconds>(curBegin.time_since_epoch()).count();
	ICC_LOG_LOWDEBUG(m_pLog, "_ReturnRConnection complete current r freesize:[%d], exec time:%llu, ", iFreeSize, ullInterval);
...
void PostgresqlPool::_CloseRConnections()
{
	...
	{
		std::vector<PooledConnectionPtr> tmpRConnections;
		...
		{
			Lock::AutoLock l_lock(m_lockSaved);
			for (auto var : tmpRConnections)
			{
				m_vecSavedRConnections.push_back(var);
			}
		}
	}
...
PooledConnectionPtr PostgresqlPool::_FindFreeRWConnection()
{
	//Lock::AutoLock l_lock(m_lockRW);

	PooledConnectionPtr pConnection = nullptr;

	std::vector<PooledConnectionPtr> vecTmp;

	for (auto var : m_RWConnections)
	{
		if (var->Get() == nullptr)
		{
			vecTmp.push_back(var);
			continue;
		}
		...
		if (!_TestConnection(var->Get()))
		{
			if (!_Reconnect(var->Get()))
			{				
				continue;
			}
		}
...
PooledConnectionPtr PostgresqlPool::_FindFreeRConnection()
{
	//Lock::AutoLock l_lock(m_lockR);
	PooledConnectionPtr pConnection = nullptr;
